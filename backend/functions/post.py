...
def fetch_multiple_old_posts(instance_url, headers, new_max_id, url_old, since_id, max_id, iterations=4, limit=40):
    ...
    if new_max_id:
        ...
        if posts_new_response.status_code == 200:
            ...
            if posts_new:
                ...
                last_new_id = int(posts_new[-1].get('id', 0))
    ...
    if posts_new_boolean and int(posts_new[-1].get('id', 0)) > int(max_id):
    ...
    for _ in range(iterations):
        ...
        if new_posts and posts_new_boolean:
            ...
            current_max_id = int(new_posts[-1].get('id', 0))  # Critical fix: Update pagination cursor
